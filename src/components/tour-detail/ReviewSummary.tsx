function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg
      className={`size-3.5 shrink-0 md:size-4 ${filled ? "fill-[#35635E] text-[#35635E]" : "fill-none text-[#0A0909]/20"}`}
      viewBox="0 0 12 12"
      stroke="currentColor"
...
type Props = {
  ratingValue?: number;
  ratingCount?: number;
  /** Compact: inline hero usage. Card: standalone card with "Read all reviews" link. */
  variant?: "inline" | "card";
  onReadAll?: () => void;
};

export function ReviewSummary({ ratingValue, ratingCount, variant = "inline", onReadAll }: Props) {
  if (ratingValue == null) return null;
  const rounded = Math.round(ratingValue);

  const stars = (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }, (_, i) => <StarIcon key={i} filled={i < rounded} />)}
    </div>
  );

  if (variant === "card") {
    return (
      <div className="rounded-2xl border border-[#0A09091A] bg-white p-4 sm:p-5">
        <p className="font-heading text-sm font-bold text-[#0A0909]">Reviews</p>
        <div className="mt-3 flex items-center gap-2">
          <span className="font-heading text-2xl font-extrabold text-[#0A0909]">{ratingValue.toFixed(1)}</span>
          {stars}
        </div>
        {ratingCount != null ?
          <p className="mt-1 text-xs text-[#0A0909]/60">Based on {ratingCount} reviews</p>
        : null}
        <button
          type="button"
          onClick={onReadAll}
          className="mt-3 text-xs font-semibold text-[#41736D] underline-offset-2 hover:underline"
        >
          Read all reviews
        </button>
      </div>
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs sm:text-[13px]">
      <span className="font-heading font-bold text-[#0A0909]">{ratingValue.toFixed(1)}</span>
      {stars}
      {ratingCount != null ?
        <span className="text-[#0A0909]/60">({ratingCount} reviews)</span>
      : null}
    </span>
  );
}
